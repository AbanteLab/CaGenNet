...
#   - Issue with convolutions
#   - Beta too large: doesn't make that much of a difference
#   - Add FC layers after convolutions and decouple location from scale earlier
# Learning noise from data
#   - Parametrizing the logscale can result in learning the noise from the data: probably good for UQ
#   - If E[X]/mode/MAP follows noise -> learning noise through E[X]/mode/MAP but not variance
# Range of data
#   - Currently, E[X]/mode/MAP is bounded between -1 and 1. This is important if we change the normalization
################################################################################################################

# VAE with convolutional layers
class ConvVAE(nn.Module):
    """
    Variational Autoencoder (VAE) for temporal fluorescence signal X that uses convolutional layers with
    circular padding to remove phase effects in the embedding.
...
###########################################################################################################
# LSTM model for temporal fluorescence signal X
###########################################################################################################
